...
function TVDetail() {
  ...
  useEffect(() => {
    const fetchTVData = async () => {
      ...
      const trailerRes = await fetch(`https://api.themoviedb.org/3/tv/${id}/videos?api_key=${TMDB_API_KEY}`);
      const trailerData = await trailerRes.json();
      const trailer = trailerData.results.find(v => v.type === "Trailer" && v.site === "YouTube");
      if (trailer) setTrailerUrl(`https://www.youtube.com/embed/${trailer.key}`);

      const castRes = await fetch(`https://api.themoviedb.org/3/tv/${id}/credits?api_key=${TMDB_API_KEY}`);
  ...
  return (
    <div className="relative min-h-screen text-white bg-black">
      <div className="absolute top-0 left-0 w-full h-full bg-cover bg-center brightness-50"
